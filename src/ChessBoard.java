...
public class ChessBoard extends JPanel implements MouseListener { // 继承面板类和鼠标事件接口
    ...
    public ChessBoard() { // 棋盘类构造函数
        ...
        addMouseMotionListener(new MouseMotionListener() { // 匿名内部类
            ...
            public void mouseMoved(MouseEvent e) { // 根据鼠标的移动所在的坐标来设置鼠标光标形状
                int x1 = (e.getX() - MARGIN + GRID_SPAN / 2) / GRID_SPAN; // 对鼠标光标的 x 坐标进行转换
                int y1 = (e.getY() - MARGIN + GRID_SPAN / 2) / GRID_SPAN; // 对鼠标光标的 y 坐标进行转换
        ...
        for (int i = 0; i < MARGIN * 2 + GRID_SPAN * COLS; i++) { // 对 board[][] 赋初值
    ...
    public void mousePressed(MouseEvent e) { // 鼠标点击事件
        ...
        xindex = (e.getX() - MARGIN + GRID_SPAN / 2) / GRID_SPAN; // 得到棋子 x 坐标
        yindex = (e.getY() - MARGIN + GRID_SPAN / 2) / GRID_SPAN; // 得到棋子 y 坐标
        board[xindex][yindex] = colorName; // 以棋子 x 坐标 y 坐标做索引将棋子的颜色添加到 board 中
        if (xindex < 0 || xindex > ROWS || yindex < 0 || yindex > COLS) { // 棋子在棋盘外不能下
            System.out.println("棋子在棋盘外不能下");
            return;
        } else if (findchess(xindex, yindex)) { // 所下位置已有棋子, 不能下
            System.out.println("该位置已有棋子, 请重新选择");
            return;
        }
        System.out.println(colorName + "落子");
    ...
    protected void paintComponent(Graphics g) { // 画棋盘和棋子
        ...
        for (int i = 0; i < chessCount; i++) { // 画棋子
            int xpos = chessList[i].getX() * GRID_SPAN + MARGIN; // 得到棋子 x 坐标
            int ypos = chessList[i].getY() * GRID_SPAN + MARGIN; // 得到棋子 y 坐标
    ...
    private boolean win(int x, int y, boolean start) { // 对棋子输赢的判断
        ...
        // 棋子所在撇行是否有五子相连的情况
        if (x + y <= 15) {
            for (int i = 0, j = x + y; i <= x + y - 4; i++, j--) {
                if (
                        isaBoolean1(i, j, str)
                ) return true;
            }
        } else {
            for (int i = x + y - 15, j = 15; i <= x + y - 4; i++, j--) {
                if (
                        isaBoolean1(i, j, str)
                ) return true;
            }
        ...
        // 棋子所在捺行是否有五子相连的情况
        if (y - x >= 0) {
            for (int i = 0, j = y - x; i <= 11 + x - y; i++, j++) {
                if (
                        isaBoolean2(i, j, str)
                )
                    return true;
    ...
    private boolean isaBoolean1(int i, int j, String str) {
        return board[i][j].equals(str)
                && board[i + 1][j - 1].equals(str)
                && board[i + 2][j - 2].equals(str)
                && board[i + 3][j - 3].equals(str)
                && board[i + 4][j - 4].equals(str);
    }

    private boolean isaBoolean2(int i, int j, String str) {
        return board[i][j].equals(str)
                && board[i + 1][j + 1].equals(str)
                && board[i + 2][j + 2].equals(str)
                && board[i + 3][j + 3].equals(str)
